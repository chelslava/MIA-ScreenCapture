...
- Поддерживает те же типы триггеров, что и CLI/API.
- Показывает preview следующих `5` запусков до сохранения задачи.
- Даёт inline validation ошибок расписания до submit.
- Поддерживает presets из `cli.templates.PRESET_TEMPLATES`.
- При редактировании показывает сохранённый `next_run` задачи.
