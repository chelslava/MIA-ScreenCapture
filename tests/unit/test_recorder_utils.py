...
class TestGetAvailableWindows:
    ...
    @patch('recorder.utils.get_platform')
    def test_exception_handling(self, mock_platform):
        """Проверка обработки исключений."""
        mock_platform.side_effect = RuntimeError("Platform error")
...
class TestGetAudioDevices:
    ...
    @patch('recorder.utils.get_platform')
    def test_audio_devices_structure(self, mock_platform):
        """Проверка структуры возвращаемых устройств."""
        mock_platform.return_value = 'windows'
        
        # Мокаем sounddevice
        with patch('recorder.utils.sd') as mock_sd:
...
class TestGetScreenSize:
    ...
    @patch('tkinter.Tk')
    def test_screen_size(self, mock_tk_class):
...
class TestValidateRectCoords:
    ...
    def test_negative_coords(self):
        ...
        # Отрицательные координаты должны быть обрезаны до 0
        assert left == 0
        assert top == 0
