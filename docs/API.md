...
REST API для удалённого управления видеозаписью экрана и приложением MIA ScreenCapture. API предоставляет полный контроль над записью видео, планированием задач и управлением конфигурацией.
...
- [Endpoints](#endpoints)
  ...
  - [Запись](#запись)
  - [Мультиисточниковая запись](#мультиисточниковая-запись)
...
Переключение источника захвата активной записи без её остановки (#48).
...
| Поле | Тип | По умолч. | Описание |
|------|-----|----------|---------|
| `area` | string | "full" | Новый тип области захвата: `full`, `window`, `rect` |
| `window_title` | string | null | Заголовок окна (требуется если `area="window"`) |
| `rect` | array | null | Координаты `[x1, y1, x2, y2]` (требуется если `area="rect"`) |
...
**Response 400:** если запись не активна или переключение не удалось.
...
curl -X POST \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"area": "window", "window_title": "Visual Studio Code"}' \
  http://localhost:5000/api/v1/recording/switch-source
```
...
Проверка целостности видеофайла по указанному пути через ffprobe (#46). Лёгкая структурная проверка — ловит, например, отсутствующий `moov atom` при неожиданном прерывании записи, без полного декодирования файла. `file_path` указывается относительно настроенного каталога вывода записей; абсолютные пути и выход за его пределы запрещены.

**Request Body:**
```json
{
  "file_path": "archive\\2024-03-15_10-30-45.mp4"
}
```
...
Если файл повреждён, `valid` будет `false`, а `error` содержит сообщение ffprobe.
...
curl -X POST \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"file_path": "archive\\2024-03-15_10-30-45.mp4"}' \
  http://localhost:5000/api/v1/recordings/verify
```
...
Попытка восстановления видеофайла ремуксом через FFmpeg (`-c copy`) (#46). Ремукс пишется во временный файл и заменяет оригинал только если сам проходит повторную проверку целостности — при любой неудаче оригинал не трогается. `file_path` указывается относительно настроенного каталога вывода записей; абсолютные пути и выход за его пределы запрещены.
...
curl -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '{"file_path": "archive\\2024-03-15_10-30-45.mp4"}' \
  http://localhost:5000/api/v1/recordings/repair
```

---

### Мультиисточниковая запись

Запись одновременно с нескольких источников (например, с двух мониторов) в отдельные файлы — каждый источник пишется независимым `VideoRecorder` со своим таймингом, без кадровой синхронизации между источниками (#51). Отдельный путь, не пересекающийся с обычной записью `/api/v1/start` — обе записи не могут быть активны одновременно.

#### `POST /api/v1/recording/start-multi`

Начало записи с нескольких источников одновременно.
...
| Поле | Тип | По умолч. | Описание |
|------|-----|----------|---------|
| `sources` | array | — | Источники захвата, минимум 2. Каждый: `label` (уникальная метка), `area` (`full`/`window`/`rect`), `monitor_index`, `window_title`, `rect` |
| `output_path` | string | null | Базовый путь вывода (каждый источник получит свой файл) |
| `fps` | number | 30 | Кадров в секунду (1-120) |
| `codec` | string | "libx264" | Видеокодек |
| `bitrate` | string | "2M" | Битрейт видео |
| `duration` | number | null | Длительность в секундах (опционально) |
...
```json
{
  ...
  "data": {
    "success": true,
    "outputs": {
      "monitor-0": "C:\\Recordings\\2024-03-15_10-30-45_monitor-0.mp4",
      "monitor-1": "C:\\Recordings\\2024-03-15_10-30-45_monitor-1.mp4"
    }
  }
}
```

**Response 400:** если обычная запись уже активна, источников меньше 2, или захват не удалось начать.
...
curl -X POST \
  ...
  -d '{
    "sources": [
      {"label": "monitor-0", "area": "full", "monitor_index": 0},
      {"label": "monitor-1", "area": "full", "monitor_index": 1}
    ],
    "fps": 30,
    "bitrate": "2M"
  }' \
  http://localhost:5000/api/v1/recording/start-multi
```

---

#### `POST /api/v1/recording/stop-multi`

Остановка мультиисточниковой записи.
...
**Response 400:** если мультиисточниковая запись не активна.

**Example:**
```bash
curl -X POST \
  -H "X-API-Key: your-api-key" \
  http://localhost:5000/api/v1/recording/stop-multi
```

---

#### `GET /api/v1/recording/status-multi`

Статус текущей мультиисточниковой записи.

**Response 200 (активна):**
```json
{
  "success": true,
  "data": {
    "active": true,
    "sources": {
      "monitor-0": {"is_recording": true, "output_path": "C:\\Recordings\\...monitor-0.mp4"},
      "monitor-1": {"is_recording": true, "output_path": "C:\\Recordings\\...monitor-1.mp4"}
    }
  }
}
```

**Response 200 (не активна):**
```json
{
  "success": true,
  "data": {
...
**Example:**
```bash
curl -H "X-API-Key: your-api-key" http://localhost:5000/api/v1/recording/status-multi
```
...
Получение текущих настроек webhook-уведомлений (без значения секрета) (#52).
...
**Example:**
```bash
curl -H "X-API-Key: your-api-key" http://localhost:5000/api/v1/config/webhook
```
...
Настройка webhook-уведомлений о завершении/ошибке записи (#52). Уведомления подписываются HMAC-SHA256 (заголовки `X-MIA-Signature`/`X-MIA-Timestamp`, TTL подписи 5 минут).

URL валидируется: должен начинаться с `http://`/`https://` и не указывать на приватный/internal-адрес (защита от SSRF, #73) — для локальной разработки используйте `allow_private_network`.
...
| Поле | Тип | По умолч. | Описание |
|------|-----|----------|---------|
| `url` | string | null | URL получателя webhook (`http://`/`https://`, не приватный адрес) |
| `secret` | string | null | Секрет для HMAC-подписи; не передавайте, чтобы оставить текущий или сгенерировать новый автоматически |
| `enabled` | bool | false | Включить отправку webhook-уведомлений |
| `allow_private_network` | bool | false | Разрешить приватные/internal-адреса в `url` (только для локальной разработки/тестов) |
...
```json
{
  ...
  "data": {
    ...
    "enabled": true,
    "has_secret": true,
    "secret": "generated-secret-shown-once"
  }
}
```

Поле `secret` присутствует в ответе только если секрет был сгенерирован автоматически в этом запросе — далее `GET /api/v1/config/webhook` не раскрывает его значение.
...
curl -X POST \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"url": "https://example.com/hook", "enabled": true}' \
  http://localhost:5000/api/v1/config/webhook
```
...
Отправка тестового webhook-уведомления (`event: "webhook.test"`) текущими настройками.
...
curl -X POST \
  -H "X-API-Key: your-api-key" \
  http://localhost:5000/api/v1/config/webhook/test
```
...
#### `GET /api/v1/resources/monitors`

Получение списка доступных мониторов для захвата (#48) — используется при выборе `monitor_index` для одиночной и мультиисточниковой записи.
...
**Response 503:** если circuit breaker для этого ресурса временно открыт (`code: "circuit_open"`) — повторите запрос позже (см. `retry_after` в `details`).

**Example:**
```bash
curl -H "X-API-Key: your-api-key" http://localhost:5000/api/v1/resources/monitors
```

---

#### `GET /api/v1/resources/disk-space`

Получение статуса свободного места на диске для текущего пути вывода записи.
...
**Example:**
```bash
curl -H "X-API-Key: your-api-key" http://localhost:5000/api/v1/resources/disk-space
```
...
Получение метрик Circuit Breaker для ресурсных эндпоинтов (`devices`, `windows`, `monitors`) — защита от каскадных сбоев при недоступности системных API захвата устройств/окон/мониторов.
...
```json
{
  ...
  "data": {
    "circuit_breakers": [
      {"name": "devices", "state": "closed", "failure_count": 0, "success_count": 142},
      {"name": "windows", "state": "closed", "failure_count": 0, "success_count": 87},
      {"name": "monitors", "state": "closed", "failure_count": 0, "success_count": 53}
    ]
  }
}
```
...
**Example:**
```bash
curl -H "X-API-Key: your-api-key" http://localhost:5000/api/v1/circuit-breakers
```
