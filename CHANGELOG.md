...
### Fixed
- Чекбоксы и радиокнопки получили явные стили индикатора состояния
  (`gui/styles/theme.py`): глобальный QSS не содержал правил
  `QCheckBox::indicator`/`QRadioButton::indicator`, из-за чего Qt рисовал
  индикатор без учёта темы, и состояние «отмечено» было практически не
  видно ни в одной из 4 тем оформления (светлая, голубая, тёмная, тёмная
