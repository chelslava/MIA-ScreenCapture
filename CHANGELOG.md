...
- Исправлен health-check rate limit (`/health`), который мог навсегда
  блокировать запрос (постоянный 429) на свежей CI-машине: проверка
  интервала использовала `time.time()` вместо `time.monotonic()` и была
  чувствительна к коррекции системных часов в первые секунды после
  старта.
