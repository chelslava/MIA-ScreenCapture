...
curl -X POST \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"file_path": "C:\\Recordings\\2024-03-15_10-30-45.mp4"}' \
...
Попытка восстановления видеофайла ремуксом через FFmpeg (`-c copy`) (#46). Ремукс пишется во временный файл и заменяет оригинал только если сам проходит повторную проверку целостности — при любой неудаче оригинал не трогается.
